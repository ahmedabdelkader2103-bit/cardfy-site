...
test('Owner UI contains only the two approved pages with RTL responsive safeguards',()=>{
  ...
  assert.match(js,/mountOwner\(value,session\)/);assert.match(js,/actor=session/);
  ...
  assert.match(html,/owner\.css\?v=20260920-kitchen-ui2/);assert.match(css,/@media\(max-width:1200px\)/);assert.match(css,/@media\(max-width:820px\)/);assert.match(css,/@media\(max-width:560px\)/);
