...
const operations=fs.readFileSync(path.join(root,'restaurant','operations.js'),'utf8');
...
test('Order Prep excludes Dine-in and cannot bypass the Kitchen ready action',()=>{
  assert.match(operations,/a\.filter\(o=>o\.order_type!=='dinein'&&group\(o\)===id\)\.length/);
  assert.match(operations,/o\.order_type!=='dinein'&&group\(o\)===bucket/);
  assert.doesNotMatch(operations,/o\.status==='preparing'\)h\+=button\('ready'/);
  assert.match(operations,/استلام وإرسال للمطبخ/);
});
