...
$('#sendRating').onclick=async()=>{if(!selectedRating){$('#ratingStatus').textContent='اختر عدد النجوم أولًا.';return}const {data,error}=await sb.rpc('cfy_os_public_feedback',{p_reference:ref,p_tracking_token:token,p_overall:selectedRating,p_food_quality:null,p_delivery_speed:null,p_packaging:null,p_feedback:$('#feedback').value.trim()});$('#ratingStatus').textContent=error||!data?'تعذر إرسال التقييم.':'شكرًا، تم حفظ تقييمك.'};
